...
<div class="container-fluid">
  <div class="row">
    ...
    <main class=" container p-5 all-content-wrapper" style="margin-left: 220px;">
        ...
        $id=$_GET['id'];
        $show= "SELECT * FROM banque WHERE id= '$id' ";
        $result=$conn->query($show);
        ?>
        <div class="row p-4 container-fluid justify-content-center">
            <div class="p-2 card bg-light col-md-10"> 
                <h4 class="text-center p-2 card" style="background-color:#32459D; color:white">Fiche d'Informations</h4> 
                <?php
            while($row = $result->fetch_assoc()){?>
                <h4>Nature: <label for=""><?=$row['nature']?></label> </h4>
                <h4>Somme Versée: <label for=""><?=$row['montantV']?> F CFA</label></h4>
                <h4>Somme Débitée: <label for=""><?=$row['montantD']?> F CFA</label></h4>
                <h4>Pieces: <label for="">N_<?=$row['numeroPiece']?></label></h4>
                <h4>Date de l'enregistrement: <label for=""><?=$row['dateB']?></label> </h4>
                <h4>Heure d'enregistrement: <label for=""> <?=$row['heureB']?></label></h4>
             <?php   } ?>
