        ...
        <div class="row p-2 container-fluid justify-content-center">
            <div class="p-4 card bg-light col-md-10"> 
                <h4 class="text-center p-1 card" style="background-color:#32459D; color: white;">Fiche d'informations sur <strong>
                        <?php $rst=$conn->query("SELECT nom FROM client WHERE id = '$id' ");
               ...
               while($row = $result->fetch_assoc()){?>
                ...
                <h4>Dernier Intervention: <label for=""><?=$row['dateIntervention']?></label></h4>
                <h4>Service Rendu: <label for=""><?=$row['intervention']?></label></h4>
                
                <div class="row justify-content-end">
                       <a href="supprimer.php?id=<?=$row['id']?>" class="btn btn-danger mr-3" ><i class="fas fa-trash-alt"> Supprimer</i></a>
                        <a href="modifier.php?id=<?=$row['id']?>" class="btn btn-primary" style="font-weight: bold;" >Modifier les informations</a>  
                </div>     
             <?php   } ?>
            </div>
        </div>
